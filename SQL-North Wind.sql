...
----clening of employee done
alter table Employees alter column BirthDate date
SELECT *, year (BirthDate) as Yearofbirth, MONTH(BirthDate) AS MonthDate
FROM Employees;
SELECT *, year (BirthDate) as Yearofbirth, MONTH(BirthDate) AS MonthDate,YEAR(GETDATE()) - YEAR(BirthDate) AS Age
FROM Employees;

---------------------
--no needed cleaning of categories
select*
from Categories

-------------
--no needed cleaning of customers
select*
from Customers
...
.........................
--no needed cleaning of orderdetails
select *from  OrderDetails
select count(distinct OrderDetailID) AS DistinctOrderDetailsCount
FROM OrderDetails;
..................
...
select count (distinct Phone) FROM Suppliers
.....................
...
making join of orderdetails&prducts then categories

solution:
SELECT*FROM Categories
SELECT SUM(Quantity *p.Price) AS REVENUEperCATEGORY,c.CategoryName
...
...........
2. How many unique products are sold per order?
SELECT COUNT(DISTINCT ProductID) AS ProductsperOrders,od.OrderID
from Orders o inner join OrderDetails od
on o.OrderID = od.OrderID
group by Od.OrderID
order by ProductsperOrders desc;
...
4. What are the total spent by each customer?
select count(od.Quantity *p.Price) as totalspent, c.CustomerName
from Orders o 
...
5. What is the average order size (quantity) per customer?
select avg(od.Quantity) as AvgOrderSize, c.CustomerName
from Orders o 
inner join OrderDetails od
on o.OrderID = od.OrderID
join Customers c
...
6. How many orders were placed in each country?
select count(distinct o.OrderID) as totalorders, c.Country
...
*7. What are the trends in customer orders over the last year?
LAST YEAR MORE THAN THE CURRENT
SELECT 
...
****8. Which employee has processed the most orders? 
...
9. Which product has the highest total sales revenue?
SELECT od.ProductID,sum( od.Quantity * p.Price) as totalrevenue, p.ProductName
from OrderDetails od join Products p
ON p.ProductID = od.ProductID
group by od.ProductID, p.ProductName
order by totalrevenue desc;

10. What are the top 5 most expensive products and which suppliers provide them?
SELECT Top 5 p.ProductName, p.Price, s.SupplierName
from Suppliers s join Products p
ON p.SupplierID = s.SupplierID
group by p.ProductName, s.SupplierName, p.Price
order by p.Price desc;

11. Which shipper is used the most for delivering orders?
SELECT s.ShipperName, COUNT(o.OrderID) AS OrdersShipped
FROM Orders o
INNER JOIN Shippers s ON o.ShipperID = s.ShipperID
GROUP BY s.ShipperName
ORDER BY OrdersShipped DESC;

12. Who are the top 5 suppliers in terms of product sales quantity?
SELECT TOP 5 s.SupplierName, SUM(od.Quantity) AS TotalQuantity
...
13. What is the average price of products by supplier?
...
relartion between  orders in each country